...
### Git / CI
- Default branch: `dev`. PRs target `dev`. Follow Conventional Commits — version bumps and GitHub Release notes are generated by release-please from commit subjects (see `CONTRIBUTING.md`). No tracked `CHANGELOG.md`. Treat this file as the canonical source for that guidance, and keep any mirrored Copilot-instructions copy in sync.
