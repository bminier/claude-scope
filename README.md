...
- **Four-column scope view** — User / User-Local / Project / Local, laid out broadest-on-the-left, with a combined-permissions view on top. The combined view is a union across scopes; it is not a full precedence-aware evaluation of what Claude Code resolves at runtime ([#30](https://github.com/bminier/claude-scope/issues/30)).
...
- On the first write of a given file per session, the original is copied to `<file>.bak`. Existing `.bak` files from previous runs are preserved, not clobbered. Backup is best-effort: a copy failure is surfaced as an error and the write is not attempted.
